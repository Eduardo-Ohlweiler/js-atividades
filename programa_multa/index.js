...
form.addEventListener("submit", (e) => {
    ...
    if (isNaN(limite) || isNaN(velocidadeVeiculo)) {
        ...
        if (
            velocidadeVeiculo <= limite) {
            situacao.innerText = "Situação: Sem multa";
        } if (
            velocidadeVeiculo > limite && velocidadeVeiculo <= limite * 1.2) {
            situacao.innerText = "Situação: Multa leve";
        } if (
             velocidadeVeiculo > limite * 1.2) {
            situacao.innerText = "Situação: Multa grave";
        } if (
            velocidadeVeiculo > limite * 2) {
            situacao.innerText = "Situação: Ai tu se passou!";
        }
